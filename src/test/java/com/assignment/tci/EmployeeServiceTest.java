...
class EmployeeServiceTest {
    ...
    void testGetBonusEligibleEmployees() throws Exception {
        ...
        List<EmployeeCurrencyGroup> result = employeeService.getBonusEligibleEmployees("may-27-2022");

        // Sort the results for consistent order
        result.sort(Comparator.comparing(EmployeeCurrencyGroup::getCurrency));

        // Debugging: print employee names
        for (EmployeeCurrencyGroup currencyGroup : result) {
        ...
        EmployeeCurrencyGroup inrGroup = result.get(0);
        EmployeeCurrencyGroup usdGroup = result.get(1);
